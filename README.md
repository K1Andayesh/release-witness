...
For judges, a clean public-demo visit opens the current model-backed appointment candidate with its exact server-managed baseline already selected. **Start 90-second tour** returns to that strongest pair, **Second workflow proof** opens the independent model-backed Fieldnotes pair, and **Benchmark receipt** exposes the portfolio totals, pair IDs, measured model contribution and aggregate digest from the same judge strip. Across both pairs, four versioned server-verified SHA-256 receipts cover report decisions, returned model, Nebius provider, timing, token totals and every stored screenshot hash. See `docs/EVALUATION.md` for scope and evidence IDs.
...
Twenty-two contract/failure tests pass. Actual Chrome verification covers the server-managed paired-run workflow, persisted pair identity, judge-tour comparison restoration after reload, receipt verification and tamper detection, release-source and commit binding, both appointment defects, the repaired candidate, before/after comparison, model-budget gating, model planning/advice, provider-failure fallback and public-demo restrictions. See `docs/QA.md`, `docs/DEMO.md` and `docs/READINESS.md`.
