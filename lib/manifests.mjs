...
export function validateManifest(value) {
  ...
  if (value.benchmark !== undefined) {
    const buildIds = new Set(value.builds.map((build) => build.id));
    const checkIds = new Set(value.checks.map((check) => check.id));
    const defectIds = value.benchmark?.knownDefectIds;
    const invariantIds = value.benchmark?.invariantIds;
    const benchmarkIds = [
      ...(Array.isArray(defectIds) ? defectIds : []),
      ...(Array.isArray(invariantIds) ? invariantIds : []),
    ];
    if (
      !buildIds.has(value.benchmark?.baselineBuild) ||
      !buildIds.has(value.benchmark?.candidateBuild) ||
      value.benchmark.baselineBuild === value.benchmark.candidateBuild ||
      !Array.isArray(defectIds) ||
      defectIds.length < 1 ||
      !Array.isArray(invariantIds) ||
      benchmarkIds.length !== value.checks.length ||
      new Set(benchmarkIds).size !== benchmarkIds.length ||
      benchmarkIds.some((id) => !checkIds.has(id))
    )
      throw new Error(
        `Manifest ${value.id} has invalid benchmark ground truth.`,
      );
  }
  if (
    !value.coverage ||
