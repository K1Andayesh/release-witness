...
export function attestBenchmark(certification, release) {
  return {
    ...
    digest: createHash("sha256")
      ...
      .digest("hex"),
    scope:
      "Benchmark ground truth, pair identities, comparison totals, verified model contribution, run receipts, exact release source and commit",
  };
}
...
export function measureModelContribution(
  ...
  const inspected = modelRuns.map((run) => {
    ...
    const planModel = run.plan?.returnedModel || run.plan?.model;
    const adviceModel = run.analysis?.returnedModel || run.analysis?.model;
    const identityVerified =
      planModel === requiredModel &&
      adviceModel === requiredModel &&
  ...
  const receiptsCoverRuntime = modelRuns.every(
    (run) => run.attestation?.schema === "run-v2",
  );
  const verified =
    modelRuns.length === 2 &&
    receiptsComplete &&
  ...
  return {
    ...
    reason: verified
      ? "Both model plans cover every allowed check, both advisories cite allow-listed evidence and both v2 run receipts cover the recorded runtime metadata."
      : modelRuns.every(
            (run) =>
              run.plan?.state !== "complete" &&
              run.analysis?.state !== "complete",
