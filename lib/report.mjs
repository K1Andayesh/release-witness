...
export function measureModelContribution(
  ...
  return {
    ...
    reason: verified
      ? "Both provider responses returned the required model with completed HTTP 200 calls; both plans cover every allowed check, both advisories cite allow-listed evidence and both v2 run receipts cover the recorded runtime metadata."
      : modelRuns.every(
            (run) =>
              run.plan?.state !== "complete" &&
              run.analysis?.state !== "complete",
