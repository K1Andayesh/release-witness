...
export async function startServer({
    ...
    try {
      ...
      if (url.pathname === "/api/benchmark/report") {
        ...
        const metric = (value, label, ariaLabel = `${label}: ${value}`) =>
          `<div class="benchmark-metric" role="group" aria-label="${escapeHtml(ariaLabel)}"><strong>${escapeHtml(value)}</strong><span>${escapeHtml(label)}</span></div>`;
  ...
  <section class="receipt-digests" aria-label="${escapeHtml(suite.name)} run receipt digests">
  ...
  <section class="suite-model-proof ${model?.verified ? "verified" : "unused"}" aria-label="${escapeHtml(suite.name)} model evidence">
