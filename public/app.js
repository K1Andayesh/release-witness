...
function render() {
  ...
  const tourGuide =
    ...
    tourPair.baseline.id === routeBaseline
      ...
      : "";
  $("#report").innerHTML =
    `<div class="report-heading"><p class="eyebrow">02 / WITNESS REPORT <span>${esc(run.id.slice(0, 8))}</span></p><h2>${verdict}</h2><p class="muted">${esc(run.targetName || "Fieldnotes")} · ${esc(run.buildLabel || run.build)} <span> / ${esc(run.suite)} / ${esc(new Date(run.createdAt).toLocaleTimeString())}${run.browser ? ` / Chrome ${esc(run.browser)}` : ""}</span></p></div>
  ...
  ${run.plan ? `<section class="plan" id="risk-map"><div class="plan-title"><h3 class="eyebrow">${run.plan.state === "complete" ? "NEMOTRON RISK MAP" : "STANDARD RISK MAP"}</h3>${run.plan.state === "complete" ? '<span class="model-proof">RECORDED MODEL CALL</span>' : ""}</div><p>${esc(run.plan.order.join(" → "))}</p><p class="fine-print">Change under review: ${esc(run.change || "Not recorded")}</p><p class="muted">${esc(run.plan.reason)}</p>${run.plan.error ? `<p class="error">${esc(run.plan.error)}</p>` : ""}${run.plan.state === "complete" ? `<p class="model-meta">${esc(run.plan.returnedModel || run.plan.model)} via ${esc(run.plan.provider)} · ${(run.plan.durationMs / 1000).toFixed(1)}s · ${run.plan.usage?.total_tokens ?? "Unknown"} tokens</p>` : ""}<p class="fine-print">${run.plan.state === "complete" ? "Nemotron chose the execution order above; Chrome ran every reviewed check in that order." : "Chrome ran every reviewed check in the standard order above."} Browser assertions determined the results; the check cards below stay in workflow order for comparison.</p></section>` : ""}
