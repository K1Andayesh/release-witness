...
- Reports are written atomically. New completed runs receive a v3 SHA-256 receipt over the full saved run record except the receipt itself, including judge-facing model text, decisions, provider response metadata and screenshot hashes. Older receipts remain readable, but only v3 records certify the model-backed portfolio. These hashes check consistency, not who produced the record.
