...
Run receipts prove that the saved report fields and screenshots still match each captured artifact set. The portfolio receipt binds the verified run-receipt digests, exact release source and Git commit to the scored benchmark, and changes if a covered result, receipt or source commit changes. These are integrity checks rather than third-party signatures.
