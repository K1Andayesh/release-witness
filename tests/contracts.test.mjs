...
test("run receipts detect decision or evidence-reference changes", () => {
  const run = {
    ...
    plan: {
      state: "complete",
      order: ["save"],
      reason: "Saved items may disappear after reload.",
    ...
    analysis: {
      ...
      action: "repeat-check",
      evidenceIds: ["save"],
      text: "Suggestion: repeat the saved-item check.",
      model: "nvidia/Nemotron-3_5-Lightning",
      provider: "Nebius Token Factory",
      durationMs: 50,
  ...
  assert.equal(run.attestation.schema, "run-v3");
  ...
  assert.equal(verifyAttestation(run), true);
  const changedAdvice = structuredClone(run);
  changedAdvice.analysis.text = "Suggestion: ship without checking.";
  assert.equal(verifyAttestation(changedAdvice), false);
  const changedReason = structuredClone(run);
  changedReason.plan.reason = "An invented explanation for the judge.";
  assert.equal(verifyAttestation(changedReason), false);
  const changedTitle = structuredClone(run);
  changedTitle.checks[0].title = "An invented check title";
  assert.equal(verifyAttestation(changedTitle), false);
  run.checks[0].observed = "Changed after receipt.";
  assert.equal(verifyAttestation(run), false);
});

test("model certification and v3 receipts require returned provider identity", () => {
  const run = {
    id: "provider-record",
    state: "complete",
    suite: "suite",
...
test("benchmark receipts bind the exact release source, commit and archive", () => {
  ...
  const first = attestBenchmark(certification, {
    ...
    commitSource:
      "https://github.com/example/release-witness/commit/1111111111111111111111111111111111111111",
    archiveSha256:
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    archiveSource:
      "https://github.com/example/release-witness/releases/download/v0.1.28/source.tar.gz",
  });
  const changedCommit = attestBenchmark(certification, {
    version: "0.1.28",
    source: "https://github.com/example/release-witness/releases/tag/v0.1.28",
    commit: "2222222222222222222222222222222222222222",
    commitSource:
      "https://github.com/example/release-witness/commit/2222222222222222222222222222222222222222",
    archiveSha256:
      "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    archiveSource:
      "https://github.com/example/release-witness/releases/download/v0.1.28/source.tar.gz",
  });
  const changedArchive = attestBenchmark(certification, {
    version: "0.1.28",
    source: "https://github.com/example/release-witness/releases/tag/v0.1.28",
    commit: "1111111111111111111111111111111111111111",
    commitSource:
...
test("server-managed pairs persist their relationship and comparison", async (t) => {
  ...
  const incompleteReadiness = await fetch(`${base}/api/readiness`);
  assert.equal(incompleteReadiness.status, 503);
  const incompleteReadinessBody = await incompleteReadiness.json();
  assert.equal(incompleteReadinessBody.ready, false);
  assert.ok(
    incompleteReadinessBody.failures.includes(
      "The benchmark is not certified.",
    ),
  );
  assert.equal(benchmark.release.version, "0.1.54");
  assert.equal(
    benchmark.release.source,
    "https://github.com/K1Andayesh/release-witness/releases/tag/v0.1.54",
  );
  assert.equal(
  ...
  assert.equal(
    benchmark.release.commitSource,
    "https://github.com/K1Andayesh/release-witness/commit/0123456789abcdef0123456789abcdef01234567",
  );
  assert.equal(
    benchmark.release.archiveSha256,
    "abcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcdefabcd",
  );
  assert.equal(
    benchmark.release.archiveSource,
    "https://github.com/K1Andayesh/release-witness/releases/download/v0.1.54/release-witness-publication-ready-v0.1.54-r1.tar.gz",
  );
  ...
  assert.match(benchmarkReportText, /Release 0\.1\.54 source/);
  ...
  const browserOnlyResponse = await fetch(`${base}/api/pairs`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Origin: base },
    body: JSON.stringify({
      suite: "booking-v1",
      change: "Exercise the public browser-only review.",
      analyze: false,
    }),
  });
  assert.equal(browserOnlyResponse.status, 202);
  let browserOnlyPair = await browserOnlyResponse.json();
  const browserOnlyDeadline = Date.now() + 30_000;
  while (
    browserOnlyPair.state === "running" &&
    Date.now() < browserOnlyDeadline
  ) {
    await new Promise((resolve) => setTimeout(resolve, 100));
    browserOnlyPair = await (
      await fetch(`${base}/api/pairs/${browserOnlyPair.id}`)
    ).json();
  }
  assert.equal(browserOnlyPair.state, "complete");
  const afterJudgeRun = await (await fetch(`${base}/api/benchmark`)).json();
  assert.equal(
    afterJudgeRun.suites.find((suite) => suite.id === "booking-v1").pairId,
    pair.id,
  );
...
test("the judge-tour comparison survives a browser reload", async (t) => {
  ...
  await page.getByRole("button", { name: /Start 90-second tour/ }).click();
  const tourHeading = await page.locator("#report h2").evaluate((element) => {
  ...
  assert.equal(tourHeading.focused, true);
  assert.equal(tourHeading.visible, true);
  assert.ok(tourHeading.scrollY > 0);
  const tour = page.getByRole("navigation", {
  ...
  assert.equal(
    await page
      .locator("#browser-checks details")
      .first()
      .evaluate((element) => element.open),
    true,
  );
  await tour.getByRole("button", { name: /Verified delta/ }).click();
  assert.equal(
    await page
      .locator("#before-after details")
      .first()
      .evaluate((element) => element.open),
    true,
  );
  assert.equal(
    await page
      .locator("#before-after details summary")
      .first()
      .evaluate((element) => element === document.activeElement),
    true,
  );
  ...
  assert.equal(deltaFocus.focused, true);
  assert.equal(deltaFocus.visible, true);
  await page.setViewportSize({ width: 390, height: 844 });
  assert.equal(
    await page.evaluate(
  ...
  assert.equal(
    await benchmarkPage
      .getByRole("link", { name: "Release 0.1.54 source ↗" })
      .getAttribute("href"),
    "https://github.com/K1Andayesh/release-witness/releases/tag/v0.1.54",
  );
  assert.equal(
  ...
  await page
    ...
    .getByText("1 concern resolved", { exact: true })
    .waitFor();
  assert.match(
    await page.locator("#comparison-summary").innerText(),
    /1 concern resolved[\s\S]*0 regressions[\s\S]*2 unchanged/,
  );
  assert.match(
    await page.locator("#comparison-result").innerText(),
    /1 concern resolved[\s\S]*2\/2 evidence receipts verified/,
  );
  assert.equal(
...
test("public demo mode excludes local projects and model spending", async (t) => {
  ...
  assert.equal(status.version, "0.1.54");
