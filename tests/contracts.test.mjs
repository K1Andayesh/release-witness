...
test("server-managed pairs persist their relationship and comparison", async (t) => {
  ...
  assert.equal(benchmark.release.version, "0.1.32");
  assert.equal(
    benchmark.release.source,
    "https://github.com/K1Andayesh/release-witness/releases/tag/v0.1.32",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "https://github.com/K1Andayesh/release-witness/releases/download/v0.1.32/release-witness-publication-ready-v0.1.32-r1.tar.gz",
  );
  ...
  assert.match(benchmarkReportText, /Release 0\.1\.32 source/);
...
test("the judge-tour comparison survives a browser reload", async (t) => {
  ...
  assert.equal(
    await benchmarkPage
      .getByRole("link", { name: "Release 0.1.32 source ↗" })
      .getAttribute("href"),
    "https://github.com/K1Andayesh/release-witness/releases/tag/v0.1.32",
  );
  assert.equal(
...
test("public demo mode excludes local projects and model spending", async (t) => {
  ...
  assert.equal(status.version, "0.1.32");
